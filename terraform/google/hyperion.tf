...
resource "template_file" "consul-ui-service" {
  template = "../consul-ui.service"
}
...
resource "google_compute_instance" "hyperion-discover" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # "sudo cat <<'EOF' > /tmp/consul-ui.service\n${template_file.consul-ui-service.rendered}\nEOF",
      # "sudo mv /tmp/consul-ui.service /lib/systemd/system/",
      "sudo systemctl daemon-reload",
      "sudo systemctl start consul.service",
      # "sudo systemctl start consul-ui.service"
    ]
  }
  depends_on = [
    "template_file.consul-service",
    # "template_file.consul-ui-service"
  ]
}
